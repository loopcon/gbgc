...
class ReportController extends Controller
{
    public function index()
    {
        ...
        return view('admin.report.list',compact('region','currencies'));
    }
    ...
    {   
        ...
        if ($request->view == 'Standard' && $request->currency == 'USD' && $request->year_from == 'all' && $request->year_to == 'all' && $request->country[0] == 'all') {
            ...
            $region=Region::orderBy('name','asc')->get();
            $currencies=Currency::get();
        } elseif ($request->view == 'Local' && $request->currency == 'LocalCurr' && $request->year_from == 'all' && $request->year_to == 'all' && $request->country[0] == 'all'){

            $yeardata = Score::where('view', 'Local')
            ->where('currency_id', 'LocalCurr')
            ->pluck('year')
            ->unique()
            ->toArray();
              ...
              foreach ($combinations as $index => $combination) {
               ...
               // Fetch scores for the current combination of levels, grouped by year and currency_id
                ...
                $scores = Score::where('view','LIKE',$viewfilter)
                    ->where('currency_id',$currencyfilter) // Add the currencies you want to include here
                    ->where('level_1', $combination->level_1)
                    ->where('level_2', $combination->level_2)
                    ->where('level_3', $combination->level_3)
                    ->where('level_4', $combination->level_4)
