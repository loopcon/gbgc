...
class ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        {
            return view('frontend.report.freeusertable',compact('region','currencies','customer'));
        }else
        {
            return view('frontend.report.index',compact('region','currencies','customer'));
        }
    ...
    public function scoreview(Request $request)
    {
        $viewfilter=$request->view;
            ...
            $yearfrom = $request->has('year_from') ? $request->year_from : 'all';
            $yearto = $request->has('year_to') ? $request->year_to : 'all';
            $country = $request->country ? $request->country : 'all';


           $yeardataQuery = Score::where('view', 'LIKE', $viewfilter)
                                ->where('currency_id', $currencyfilter);
            if ($yearfrom !== 'all' && $yearto !== 'all') {
                    $yeardataQuery->whereBetween('year', [$yearfrom, $yearto]);
            }
            if ($country !== 'all') {
                    $countryIds = explode(',', $country);
                    $yeardataQuery->whereIn('region_id', $countryIds);
            }
            $yeardata = $yeardataQuery->pluck('year')->unique()->toArray();
            $exportData = [];
            ...
            $combinations = Score::with('maincategoryDetail','subcategory1Detail','subcategory2Detail','level4Detail')
              ...
              foreach ($combinations as $index => $combination) {
               ...
               // Fetch scores for the current combination of levels, grouped by year and currency_id
                ...
                $scores = Score::where('view', 'LIKE', $viewfilter)
                        ->where('currency_id', $currencyfilter)
                ...
                if ($yearfrom !== 'all' && $yearto !== 'all') {
                    $scores->whereBetween('year', [$yearfrom, $yearto]);
                }

                // Add country condition
                if ($country !== 'all') {
                    $countryIds = explode(',', $country);
                    $scores->whereIn('region_id', $countryIds);
                }

                $scores = $scores->groupBy('year', 'currency_id')
                    ->selectRaw('year, currency_id, SUM(score) as total_score')
